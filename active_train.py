...
sys.path.append("./")
from radiative_gaussian.arguments import ModelParams, OptimizationParams, PipelineParams
from radiative_gaussian.gaussian import GaussianModel, render, query, initialize_gaussian
from radiative_gaussian.utils.general_utils import safe_state
from radiative_gaussian.utils.cfg_utils import load_config
from radiative_gaussian.utils.log_utils import prepare_output_and_logger
from radiative_gaussian.dataset import Scene
from radiative_gaussian.utils.loss_utils import l1_loss, ssim, tv_3d_loss
from radiative_gaussian.utils.image_utils import metric_vol, metric_proj
from radiative_gaussian.utils.plot_utils import show_two_slice

from radiative_gaussian.active.schema import generate_schema
from radiative_gaussian.active import methods_dict
...
def training(
    ...
    # Active View Selection Set-up
    schema = generate_schema(args.schema, dataset_size=len(scene.getTrainCameras()), scene=scene)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        # Get the num views left to add
        if iteration == opt.iterations:
            # No more views to add
            # Write the list of training views ids into a yml file
            if not os.path.exists(osp.join(scene.model_path, "train_views.yml")):
                with open(osp.join(scene.model_path, "train_views.yml"), "w") as f:
                    yaml.dump(
                        {"train_views": scene.train_idxs},
                        f,
        ...
        if num_views > 0:
            try:
                ...
                selected_views = active_method.nbvs(gaussians, scene, num_views, pipe)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[5_000, 10_000, 20_000, 30_000, 35_000, 40_000, 45_000])
    ...
    parser.add_argument("--iqa_batch_size", type=int, default=8)

    parser.add_argument("--reg_lambda", type=float, default=1e-6)
    parser.add_argument("--I_test", action="store_true")
    parser.add_argument("--I_acq_reg", action="store_true")
