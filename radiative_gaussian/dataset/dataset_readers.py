...
sys.path.append("./")
from radiative_gaussian.utils.graphics_utils import BasicPointCloud, fetchPly
...
def readCTameras(meta_data, source_path, eval=False, scene_scale=1.0):
    ...
    for split in splits:
        ...
        for i_split in range(n_split):
            ...
            frame_lat = frame_info.get("lat", None)
            frame_distance_scale = float(frame_info.get("distance_scale", 1.0))

            # CT 'transform_matrix' is a camera-to-world transform
            if frame_lat is None:
                c2w = angle2pose(
                    cam_cfg["DSO"] * frame_distance_scale, frame_angle
                )  # c2w
                lon = frame_angle
                lat = 0.0
            else:
                lon = frame_angle
                lat = frame_lat
                radius = float(cam_cfg["DSO"]) * frame_distance_scale
                pos = np.array(
                    [
                ...
                c2w_equator = angle2pose(radius, lon)
                ref_right = c2w_equator[:3, 0]
                ref_up = c2w_equator[:3, 1]
                forward = target - pos
                ...
                right = ref_right - forward * np.dot(ref_right, forward)
                ...
                # Vertical flip only (up-down), keep right as-is.
                up = -up
